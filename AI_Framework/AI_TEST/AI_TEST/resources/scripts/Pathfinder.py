...
class Pathfinder:
	...
	#If the pathfinder hasn't been started yet...
		...
		#Add a COPY of the starting cell to the open list
			self.openList.append(copy(self.startCell))
			for i in range (0, len(self.startCell.neighbors)):
				self.openList.append(copy(self.startCell.neighbors[i]))
		else:
			pass
	...
	#Primary loop, will repeat the A* Pathfinding sequence until the target cell is added to the closed list.
		...
		print "Lowest F Score: " + str(self.openList[0].f)
		...
		for i in range (0, len(self.currentCell.neighbors)):
			...
			if self.currentCell.neighbors[i].InOpenList() == False:
