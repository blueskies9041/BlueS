...
void PythonManager::Initialize(char * a_pScriptDirectory)
{
	Py_Initialize(); 
...
void PythonManager::ImportScript(char * a_pScriptName)
{
...
void PythonManager::ReloadScript(char * a_pScriptName)
{
	m_Scripts[a_pScriptName] = PyImport_ReloadModule(m_Scripts[a_pScriptName]);
}

void PythonManager::AddTable(char * a_pTableName, PyMethodDef * a_pyFunctionTable) 
{
	Py_InitModule(a_pTableName , a_pyFunctionTable ); 
}

PyObject * PythonManager::GetFunction(char * a_pScriptName, char * a_pFunctionName)
{
...
PyObject * PythonManager::CallFunction(PyObject * a_pyFunction, char * a_pFormat, ...)
{
	if(a_pFormat == NULL)
	{
